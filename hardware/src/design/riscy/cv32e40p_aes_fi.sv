...
////////////////////////////////////////////////////////////////////////////////
// Design Name:    AES32 scalar-crypto final-round unit                       //
// Project Name:   RI5CY                                                      //
// Language:       SystemVerilog                                              //
//                                                                            //
// Description:    Combinational implementation of the RISC-V scalar          //
//                 cryptography extension instruction aes32esi (Zkne).        //
//                                                                            //
//                 rd = rs1 ^ rot_left( zext32( SBox( rs2[bs] ) ), bs * 8 )   //
//                                                                            //
//                 Same as aes32esmi but WITHOUT MixColumns - the sbox byte   //
//                 is placed in byte position bs of an otherwise-zero word.   //
////////////////////////////////////////////////////////////////////////////////

module cv32e40p_aes_fi
(
    ...
    // AES forward S-box (identical LUT to cv32e40p_aes.sv)
    logic [7:0] sb_out;
    always_comb begin
        unique case (sb_in)
    ...
    // No MixColumns: zero-extend the sbox byte into a 32-bit word at byte 0.
    logic [31:0] sb_word;
    assign sb_word = {24'h0, sb_out};

    // Rotate-left by bs*8 places the sbox byte at byte position bs (others = 0).
    logic [31:0] rotated;
    always_comb begin
        unique case (bs_i)
            2'd0: rotated =  sb_word;
            2'd1: rotated = {sb_word[23:0], sb_word[31:24]};
            2'd2: rotated = {sb_word[15:0], sb_word[31:16]};
            2'd3: rotated = {sb_word[ 7:0], sb_word[31: 8]};
        endcase
    end
